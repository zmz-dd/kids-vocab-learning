...
export default function Mistakes() {
  ...
  return (
    <div className="min-h-screen p-4 flex flex-col max-w-md mx-auto bg-red-50/30">
      ...
      <Tabs value={activeTab} onValueChange={(v) => { setActiveTab(v); setReviewIndex(0); setShowMeaning(false); }} className="flex-1 flex flex-col">
          <TabsList className="grid w-full grid-cols-3 h-14 bg-white/50 p-1 rounded-2xl mb-6 shadow-sm border border-white">
              <TabsTrigger value="all" className="rounded-xl font-bold text-xs">All</TabsTrigger>
              <TabsTrigger value="today" className="rounded-xl font-bold text-xs">Today</TabsTrigger>
              <TabsTrigger value="rank" className="rounded-xl font-bold text-xs">Rank</TabsTrigger>
          </TabsList>

          <TabsContent value="all" className="flex-1 flex flex-col mt-0">
              <MistakesCardView mode="all" />
          </TabsContent>
          <TabsContent value="today" className="flex-1 flex flex-col mt-0">
              <MistakesCardView mode="today" />
          </TabsContent>

          <TabsContent value="rank" className="flex-1 mt-0 overflow-hidden flex flex-col">
              <div className="bg-white/50 p-4 rounded-t-3xl border-b border-white/20 flex items-center justify-between">
  ...
  function MistakesCardView({ mode }: { mode: 'all' | 'today' }) {
      ...
      return (
        <>
          ...
          <div className="grid grid-cols-1 gap-4 mt-8 pb-6 px-2">
